...
const ActionsMenu = ({ onEdit, onDelete }: { onEdit: (e: React.MouseEvent) => void, onDelete: (e: React.MouseEvent) => void }) => {
      ...
      {isOpen && (
        <div className="absolute right-0 mt-2 w-32 rounded-md shadow-lg bg-slate-800 ring-1 ring-black ring-opacity-5 z-10">
          <div className="py-1" role="menu" aria-orientation="vertical" aria-labelledby="options-menu">
            <button onClick={onEdit} className="block w-full text-left px-4 py-2 text-sm text-slate-300 hover:bg-slate-700">Edit</button>
            <button onClick={onDelete} className="block w-full text-left px-4 py-2 text-sm text-red-400 hover:bg-slate-700">Delete</button>
          </div>
        </div>
      )}
    </div>
...
export default function BrandCard({ brand, onEdit, onDelete }: BrandCardProps) {
  return (
    <Link href={`/brand/${brand.id}`} className="block h-full">
        <Card className="flex flex-col h-full group transition-all duration-300 hover:border-primary/80 hover:shadow-primary/20 cursor-pointer">
