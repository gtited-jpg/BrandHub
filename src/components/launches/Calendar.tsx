...
export default function Calendar({ launches, brands, onEditLaunch, onDeleteLaunch }: CalendarProps) {
  ...
  const launchesByDate = useMemo(() => {
    ...
    launches.forEach(launch => {
      const dateKey = format(new Date(launch.start_date), 'yyyy-MM-dd');
  ...
  const brandColorMap = useMemo(() => new Map(brands.map(b => [b.id, b.primary_color || '#8b5cf6'])), [brands]);
    ...
    <div className="bg-slate-800/50 rounded-lg border border-slate-700 p-4">
      ...
      <div className="grid grid-cols-7">
        {days.map((d) => (
          <div
            key={d.toString()}
            className={`h-40 border border-slate-700/50 p-2 overflow-y-auto ${!isSameMonth(d, monthStart) ? 'bg-slate-900/40' : ''}`}
          >
